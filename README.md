...
This project started as handy bash script to setup my $masternodes in 2016 when there was almost zero documentation and anything that existed was either $DASH specific, sucked and in most cases both. For that reason, i started to work on a not-so-sucking way to install a lot of different masternodes with next to none manual intervention.
...
```bash
git clone https://github.com/TheCryptoR/vps.git && cd vps
```
...
**Install & configure 4 CTS masternodes:**

```bash
./install.sh -p cts -c 4
```

**Update daemon of previously installed CTS masternodes:**

```bash
./install.sh -p cts -u
```

**Install 6 CTS masternodes with the git release tag "tags/v3.0.0.1"**

```bash
./install.sh -p cts -c 6 -r "tags/v3.0.0.1"
```

**Wipe all CTS masternode data:**

```bash
./install.sh -p cts -w
```

**Install 2 CTS masternodes and configure sentinel monitoring:**

```bash
./install.sh -p cts -c 2 -s
```
...
If you want to check the status of your masternode, the best way is currently running the cli e.g. for $CTS via

```
edit config: nano /etc/masternodes/cts_n1.conf
 
getinfo: /usr/local/bin/cts-cli -conf=/etc/masternodes/cts_n1.conf getinfo

Masternode Status: /usr/local/bin/cts-cli -conf=/etc/masternodes/cts_n1.conf masternode status

stop daemon: /usr/local/bin/cts-cli -conf=/etc/masternodes/cts_n1.conf stop

start daemon: /usr/local/bin/activate_masternodes_cts

Output /usr/local/bin/cts-cli -conf=/etc/masternodes/cts_n1.conf getinfo
